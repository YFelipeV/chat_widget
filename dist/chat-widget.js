import { jsxs as a, jsx as e, Fragment as w } from "react/jsx-runtime";
import { useState as m, useRef as j, useEffect as f } from "react";
const q = "data:image/png;base64,", U = ({ onQuestionClick: n }) => /* @__PURE__ */ a("div", { className: "frequent-questions", children: [
  /* @__PURE__ */ e("h3", { children: "Preguntas Frecuentes" }),
  /* @__PURE__ */ e("ul", { children: [
    "Por favor muestra una tabla con los datos principales de las sucursales registradas.",
    "¿Cuáles son los servicios disponibles en PARE?",
    "¿Cómo puedo contactar a soporte técnico?"
  ].map((o, c) => /* @__PURE__ */ e("li", { onClick: () => n(o), children: o }, c)) })
] }), E = ({ message: n }) => /* @__PURE__ */ e("div", { className: `message ${n.isUser ? "user-message" : "bot-message"}`, children: /* @__PURE__ */ e("div", { className: "message-content-wrapper", children: /* @__PURE__ */ e("div", { className: "message-content", dangerouslySetInnerHTML: { __html: n.text } }) }) }), b = () => {
  const [n, i] = m([]), [o, c] = m(""), [d, p] = m(!1), [h, S] = m(!0), u = j(null);
  f(() => {
    v();
  }, []);
  const v = async () => {
    try {
      const s = await getChatHistory();
      if (s.STATUS) {
        const A = s.RSP.map((t) => ({
          id: t.id,
          text: t.message,
          isUser: t.role === "user",
          status: "sent",
          timestamp: new Date(t.created_at)
        }));
        i(A);
      }
    } catch (s) {
      console.error("Error cargando historial:", s);
    } finally {
      S(!1);
    }
  }, C = () => {
    if (u.current) {
      const { scrollHeight: s } = u.current;
      u.current.scrollTo({
        top: s,
        behavior: "smooth"
      });
    }
  };
  f(() => {
    C();
  }, [n, d]);
  const g = async (s = o) => {
    if (s.trim() === "") return;
    const A = {
      id: Date.now().toString(),
      text: s,
      isUser: !0,
      status: "sent",
      timestamp: /* @__PURE__ */ new Date()
    };
    i((t) => [...t, A]), c(""), p(!0);
    try {
      const t = await sendMessageApi({ message: s });
      if (t.STATUS) {
        const l = {
          id: (Date.now() + 1).toString(),
          text: t.RSP,
          isUser: !1,
          status: "sent",
          timestamp: /* @__PURE__ */ new Date()
        };
        i((r) => [...r, l]);
      } else {
        c(s);
        const l = {
          id: (Date.now() + 1).toString(),
          text: "<p>Error, Intenta nuevamente</p>",
          isUser: !1,
          status: "sent",
          timestamp: /* @__PURE__ */ new Date()
        };
        i((r) => [...r, l]);
      }
    } catch {
      i(
        (l) => l.map(
          (r) => r.id === A.id ? { ...r, status: "error" } : r
        )
      );
    } finally {
      p(!1);
    }
  }, I = (s) => {
    s.preventDefault(), g();
  }, N = (s) => {
    g(s);
  };
  return /* @__PURE__ */ a("div", { className: "chat-boot-container", children: [
    /* @__PURE__ */ a("div", { className: "chat-header", children: [
      /* @__PURE__ */ e("div", { children: /* @__PURE__ */ e("img", { src: q, className: "icon-header", alt: "Logo Corbot" }) }),
      /* @__PURE__ */ e("div", { children: /* @__PURE__ */ e("span", { className: "title-header", children: "Cor-IA" }) })
    ] }),
    /* @__PURE__ */ e(
      "div",
      {
        ref: u,
        className: "messages-container",
        children: h ? /* @__PURE__ */ a("div", { className: "loading-container", children: [
          /* @__PURE__ */ a("div", { className: "loading-dots", children: [
            /* @__PURE__ */ e("div", { className: "dot" }),
            /* @__PURE__ */ e("div", { className: "dot" }),
            /* @__PURE__ */ e("div", { className: "dot" })
          ] }),
          /* @__PURE__ */ e("span", { className: "loading-text", children: "Cargando mensajes" })
        ] }) : /* @__PURE__ */ a(w, { children: [
          n.length === 0 && /* @__PURE__ */ e(U, { onQuestionClick: N }),
          n.map((s) => /* @__PURE__ */ e(E, { message: s }, s.id)),
          d && /* @__PURE__ */ a("div", { className: "typing-indicator", children: [
            /* @__PURE__ */ e("span", {}),
            /* @__PURE__ */ e("span", {}),
            /* @__PURE__ */ e("span", {})
          ] })
        ] })
      }
    ),
    /* @__PURE__ */ a("form", { onSubmit: I, className: "message-input", children: [
      /* @__PURE__ */ e(
        "input",
        {
          type: "text",
          value: o,
          onChange: (s) => c(s.target.value),
          placeholder: "Escribe tu mensaje...",
          disabled: d || h,
          className: "message-input-field"
        }
      ),
      /* @__PURE__ */ e(
        "button",
        {
          type: "submit",
          className: "send-button",
          disabled: d || h || o.trim() === "",
          children: /* @__PURE__ */ e("i", { className: "ri-send-plane-fill" })
        }
      )
    ] })
  ] });
}, y = () => /* @__PURE__ */ e("div", { children: /* @__PURE__ */ e("div", { className: "chat-toogle-content", children: /* @__PURE__ */ e(b, {}) }) });
export {
  y as ChatWidget
};
